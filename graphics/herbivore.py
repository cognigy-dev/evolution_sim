...
class Herbivore:
    def __init__(self, x, y):
        ...
        # Load the herbivore image from graphics/assets
        script_dir = os.path.dirname(os.path.abspath(__file__))
        image_path = os.path.join(script_dir, 'assets', 'herbivore.png')
        log_error(f"Trying to load herbivore image from: {os.path.abspath(image_path)}")
        ...
        try:
            ...
            self.has_image = True
            log_error("Successfully loaded herbivore image")
        except Exception as e:
            log_error(f"Error loading herbivore image from {image_path}: {e}")
