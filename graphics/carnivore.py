    ...
    def __init__(self, x, y):
        ...
        try:
            self.base_image = pygame.transform.scale(
                pygame.image.load(os.path.join(script_dir, 'assets', 'carnivore.png')).convert_alpha(),
                (self.size, self.size)
            )
            self.image = self.base_image
            self.has_image = True
        except Exception as e:
            print(f"Error loading carnivore image: {e}")
            self.has_image = False

    def update(self):
        if self.lerp_t >= 1.0:  # If not currently moving
            if random.random() < 0.04:  # 4% chance to move
